...
ABOUT_TEXT = (
    ...
    "(по официальному подтверждению от Twitch, а не догадке по всплеску в чате).\n\n"
    ...
    "🚨 <b>Алерты</b> — сообщаю, если канал забанен/удалён на Twitch или сменил имя.\n\n"
    "💬 <b>Топ чатеров</b> — самые активные зрители чата за стрим.\n\n"
    "🤝 <b>Тег коллаба</b> — если в названии стрима есть другой отслеживаемый канал, "
    "помечаю отчёт как совместный стрим.\n\n"
    "📍 <b>Гибкая доставка</b> — итоговый отчёт можно получать в группу или себе в личку.\n\n"
    "🌙 <b>Тихие часы</b> — задай период (например, 23:00–08:00), и отчёты в личку "
    "в это время соберутся в одну сводку и придут после его окончания.\n\n"
    "📊 <b>Итоговый отчёт</b> — после стрима присылаю сводку: длительность, пик и среднее "
    "число зрителей, новых фолловеров. Есть краткий (текст) и развёрнутый формат — "
    "с графиком, клипами, списком чатеров и ссылкой на VOD с таймкодами.\n\n"
    "📌 <b>Для стримеров и админов групп</b> — подключи канал в группе, и уведомления "
    "будут публиковаться полностью автоматически, а после стрима — сами удаляться.\n\n"
    "⚙️ <b>Удалённое управление</b> — админ группы может менять её настройки прямо "
    "из личного чата с ботом.\n\n"
    "🔐 <b>Подключение Twitch-аккаунта</b> — команда /auth_twitch открывает доступ "
    "к числу фолловеров канала в отчёте."
)
...
async def cmd_stats(message: Message, db: Database, config: Config) -> None:
    ...
    await message.answer(
        "📈 <b>Статистика бота</b>\n\n"
        f"👤 Личных пользователей: <b>{stats['private_users']}</b>\n"
